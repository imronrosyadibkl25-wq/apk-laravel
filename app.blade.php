        ...
        <div class="navbar-links" id="navbarLinks">
            ...
            <div class="nav-user" style="display: flex; align-items: center; gap: 0.8rem; padding: 0.3rem 1rem;">
                <div style="display: flex; flex-direction: column; align-items: flex-start; line-height: 1.2;">
                    <span style="font-size: 0.85rem; font-weight: 700;">{{ Auth::user()->name }}</span>
                    <span style="font-size: 0.75rem; font-weight: 600; color: var(--pink);">{{ Auth::user()->levelTitle() }}</span>
                </div>
                <div style="display: flex; flex-direction: column; align-items: flex-end; line-height: 1.2;">
                    <span class="badge-level" style="background: var(--purple); color: var(--white); border: 2px solid var(--black); border-radius: 4px; padding: 0.05rem 0.3rem; font-size: 0.75rem; font-weight: 800;">Lvl {{ Auth::user()->level }}</span>
                    <div style="width: 60px; height: 6px; background: #e0e0e0; border: 1px solid var(--black); border-radius: 3px; overflow: hidden; margin-top: 0.2rem;" title="XP: {{ Auth::user()->xp }} / {{ Auth::user()->xpForNextLevel() }}">
                        <div style="width: {{ Auth::user()->xpPercentage() }}%; height: 100%; background: var(--green);"></div>
                    </div>
                </div>
            </div>
